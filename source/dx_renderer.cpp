...
void DXRenderer::initSwapChain(HWND hwnd, int width, int height)
{
    swapChain = createSwapChain(hwnd, width, height);
}
...
void DXRenderer::populateCommandList(std::vector<RenderCommand> commands)
{
    ...
    result = commandList->Reset(commandAllocator.Get(), NULL);
    ...
    scissorRect.bottom = static_cast<LONG>(500);
